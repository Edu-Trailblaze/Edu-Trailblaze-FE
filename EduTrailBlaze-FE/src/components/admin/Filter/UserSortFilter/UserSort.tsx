...
import { User } from '@/app/(Admin)/admin_dashboard/Dashboard/users/page'

interface UserSortProps {
  columns: { label: string; accessor: keyof User }[]
  visibleColumns: Record<keyof User, boolean>
  onApply: (newVisibleColumns: Record<keyof User, boolean>) => void
  onClose: () => void
  onClear: () => void
}

const UserSort: React.FC<UserSortProps> = ({ columns, visibleColumns, onApply, onClose, onClear }) => {
